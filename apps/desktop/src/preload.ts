...
contextBridge.exposeInMainWorld('electronAPI', {
  // File processing
  processFileContent: (filePath: string) => ipcRenderer.invoke('process-file-content', filePath),
  ...
  // Event listeners
  onNewFileDetected: (callback: (data: { filePath: string; aiResult: any }) => void) => {
...
window.addEventListener('DOMContentLoaded', () => {
  console.log('🔍 PRELOAD: DOM loaded, electronAPI available:', typeof (window as any).electronAPI);
});
...
export interface ElectronAPI {
  ...
  onNewFileDetected: (callback: (data: { filePath: string; aiResult: any }) => void) => void;
